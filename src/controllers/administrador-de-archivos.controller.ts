...
export class AdministradorDeArchivosController {
  ...
  async CargarArchivoProducto(
    ...
    const filePath = path.join(__dirname, ConfiguracionGeneral.cargarArchivosProductos);
    let res = await this.StoreFilePath(
      filePath,
      ConfiguracionGeneral.campoDeProductos,
      request,
      response,
  ...
  private ObtenerArchivosPorTipo(tipo: number) {
    ...
    switch (tipo) {
      case 1:
        filePath = path.join(__dirname, ConfiguracionGeneral.cargarArchivosProductos);
        break;
      case 2:
        filePath = path.join(__dirname, ConfiguracionGeneral.cargarArchivosClientes);
        break;
      case 3:
        break;
    }
  ...
  /**
   * Validate file name to prevent them goes beyond the designated directory
